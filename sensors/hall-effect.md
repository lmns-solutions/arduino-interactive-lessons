...
1. Just like with our {% include tlink.html id='sensors-photoresistors' text='photoresistor' %}#An-improved-auto-on-nightlight-circuit circuit, we could change our circuit to use a **transistor**. In this case, the Hall effect sensor output would be connected to a transistor, which would control the current through our LED. If you have a transistor, feel free to try this!
