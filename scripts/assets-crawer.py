...
Example usage:
  python jekyll-missing-assets-fetcher.py \
    --root . \
...
Notes
-----
- The script extracts URLs from Markdown image/link syntax and from HTML tags
  like <img>, <video>, and <source>.
- It only considers references whose paths contain "assets/".
- For each missing local asset, the script will try downloading it from a set of
  candidate remote URLs composed as:
    remote_base + prefix + /assets/...
  where "prefix" is each item from --remote-try-prefixes. The literal token
  {section} (if present) is replaced by the first-level folder of the markdown
  file (e.g., electronics, esp32) when available.
- If a referenced URL is already an absolute HTTP(S) URL and not pointing to
  localhost, the script attempts that first before trying the remote prefixes.
...
MARKDOWN_URL_PATTERNS = [
    # ![alt](url "title") or [text](url)
    re.compile(r"!\[[^\]]*\]\((?P<url>[^\)\s]+)(?:\s+\"[^\"]*\")?\)", re.IGNORECASE),
    re.compile(r"\[[^\]]*\]\((?P<url>[^\)\s]+)(?:\s+\"[^\"]*\")?\)", re.IGNORECASE),
]

HTML_SRC_PATTERNS = [
    re.compile(r"<img[^>]*?\ssrc=\"(?P<url>[^\"]+)\"", re.IGNORECASE),
    re.compile(r"<source[^>]*?\ssrc=\"(?P<url>[^\"]+)\"", re.IGNORECASE),
    re.compile(r"<video[^>]*?\ssrc=\"(?P<url>[^\"]+)\"", re.IGNORECASE),
]
...
def normalize_asset_path(url: str, local_baseurl: str) -> Optional[str]:
    ...
    - Accepts absolute http(s) URLs, site-root absolute ("/..."), or relative ("assets/...").
    - Strips the local_baseurl prefix if present.
    - Ensures the returned path starts with "/assets/".
    """
    url = unquote(url).strip()

    # Handle Jekyll/Liquid patterns like: {{ '/assets/x.mp4' | relative_url }}
    ...
    if not norm.startswith("/assets/"):
        # e.g., "/something/assets/..." -> force from the keyword
        norm = "/" + path[idx:]
    return norm
...
def guess_section_for_file(md_path: Path, root: Path) -> Optional[str]:
    try:
        ...
        rel = md_path
    parts = rel.parts
    # If file is at <root>/<section>/something.md, return that first folder.
    if len(parts) >= 2:
        first = parts[0]
        # avoid considering the global assets folder as a content section
...
def candidate_download_urls(asset_path: str, *,
    ...
    for p in prefixes:
        ...
        if "{section}" in p and section:
            ...
            # If section is missing, skip this template.
            continue
