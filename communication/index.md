...
## {% include tlink.html id='communication-serial-intro' text='L1: Intro to Serial' %}

In {% include tlink.html id='communication-serial-intro' text='this lesson' %}, we’ll dive into asynchronous serial communication and how we can use it for bidrectional `Computer ↔ Arduino` communication. We'll show example serial communication clients using terminal programs and Python.

## {% include tlink.html id='communication-web-serial' text='L2: Web Serial' %}

In {% include tlink.html id='communication-web-serial' text='this lesson' %}, you'll learn about the new [Web Serial API](https://wicg.github.io/serial/) and how to build simple web apps that communicate with Arduino.

## {% include tlink.html id='communication-p5js-serial' text='L3: p5.js Serial' %}

In {% include tlink.html id='communication-p5js-serial' text='this lesson' %}, you'll learn about [Processing](https://processing.org/), [p5.js](https://p5js.org/), and how to use p5.js with Web Serial (with a focus on processing incoming serial input data).

## {% include tlink.html id='communication-p5js-serial-io' text='L4: p5.js Serial I/O' %}

In {% include tlink.html id='communication-p5js-serial-io' text='this lesson' %}, we look at creating more complicated p5.js + Arduino applications that bidirectionally communicate together both Arduino to p5.js (`Arduino → Computer`) as well as p5.js to Arduino (`Computer → Arduino`).

## {% include tlink.html id='communication-p5js-paint-io' text='L5: Paint I/O Example' %}

In {% include tlink.html id='communication-p5js-paint-io' text='this lesson' %}, we will bring everything together thus far to build a fully interactive p5.js + Arduino painting application, called PaintIO, with bidirectional serial communication and a custom paintbrush controller using an accelerometer to control brush location, a {% include tlink.html id='arduino-force-sensitive-resistors' text='force-sensitive resistor' %} to control brush size, and bimanual interaction using both Arduino + the mouse simultaneously for drawing.

## {% include tlink.html id='communication-ml5js-serial' text='L6: ml5.js Serial' %}

In {% include tlink.html id='communication-ml5js-serial' text='this lesson' %}, we'll introduce machine learning frameworks like [Runway ML](https://runwayml.com/) and [ml5js](https://ml5js.org/) and show how to use them with Arduino.
